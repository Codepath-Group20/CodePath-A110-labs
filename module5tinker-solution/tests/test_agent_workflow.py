...
class _BadSeverityClient:
    """A fake client that returns valid JSON but with an out-of-contract severity."""

    def complete(self, system_prompt: str, user_prompt: str) -> str:
        return '[{"type": "Reliability", "severity": "CRITICAL", "msg": "boom"}]'
...
def test_invalid_severity_forces_fallback_to_heuristics():
    # [Part 2 change] A parseable-but-out-of-contract severity ("CRITICAL")
    # should be rejected and the agent should fall back to the heuristic
    # analyzer instead of trusting the bad value.
    agent = BugHoundAgent(client=_BadSeverityClient())
    code = "def f():\n    print('hi')\n    return True\n"
    result = agent.run(code)

    # Heuristics detected the print -> Code Quality (Low), not the bogus CRITICAL.
    assert any(issue.get("type") == "Code Quality" for issue in result["issues"])
    assert all(
        str(issue.get("severity", "")).lower() in {"low", "medium", "high"}
        for issue in result["issues"]
    )
    assert any("invalid severity" in entry.get("message", "").lower() for entry in result["logs"])
